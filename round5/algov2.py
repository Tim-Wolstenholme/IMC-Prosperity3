...
class Trader:
    ...
    def buy_artificial_croissant(self,state, market_data, etf):
        ...
        amount = min(-sum(market_data.ask_volumes[etf]), sum(market_data.bid_volumes["JAMS"])/component_quantities["JAMS"])
    ...
    def sell_artificial_croissant(self,state, market_data, etf):
        ...
        amount = min(sum(market_data.bid_volumes[etf]), -sum(market_data.ask_volumes["JAMS"])/component_quantities["JAMS"])
        if component_quantities.get("DJEMBES",0) != 0:
    ...
    def olivia_croissant(self, state, market_data,traderObject):
        ...
        for trade in trades:
            if trade.timestamp == state.timestamp -100:
                if trade.buyer == "Olivia" or traderObject["etf_direction"] == 1: # Nancy Pelosi moment
                    print("LONGING")
                    ...
                    if market_data.buy_sum[product] == 0:
                        traderObject["etf_direction"] = 0
                elif (trade.seller == "Olivia") or (traderObject["etf_direction"] == -1):
                    print("SHORT")
                    traderObject["etf_direction"] = -1
    ...
    def olivia_ink(self, state, market_data,traderObject):
        product = "SQUID_INK"
        orders = []
        fair = market_data.fair[product]
        if "squid_direction" not in traderObject:
            traderObject["squid_direction"] = 0
        trades = state.market_trades.get(product, [])
        for trade in trades:
            if trade.timestamp == state.timestamp -100:
                if trade.buyer == "Olivia" or traderObject["squid_direction"] == 1: # Nancy Pelosi moment
                    fill = min(-market_data.ask_volumes[product][0], market_data.buy_sum[product])
                    orders.append(Order(product, market_data.ask_prices[product][0], fill))
                    traderObject["squid_direction"] = 1
                    market_data.buy_sum[product] -= fill
                    if market_data.buy_sum[product] ==0:
                        traderObject["squid_direction"] = 0
                elif trade.seller == "Olivia" or traderObject["squid_direction"] == -1:
                    fill = min(market_data.bid_volumes[product][0], market_data.sell_sum[product])
    ...
    def run(self, state: TradingState):
        ...
        result["SQUID_INK"] = self.olivia_ink(state, market_data,traderObject)
