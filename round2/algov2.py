...
class Trader:
    ...
    def basket1_fair_value(self, order_depth: OrderDepth, traderObject,
                           b2_order_depth: OrderDepth):
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            best_ask = min(order_depth.sell_orders.keys())
            best_bid = max(order_depth.buy_orders.keys())
            ...
            valid_ask = [price for price in order_depth.sell_orders.keys()
                         if
                         abs(order_depth.sell_orders[price]) >= self.params[Product.PICNIC_BASKET1]["adverse_volume"]]
            valid_buy = [price for price in order_depth.buy_orders.keys()
                         if abs(order_depth.buy_orders[price]) >= self.params[Product.PICNIC_BASKET1]["adverse_volume"]]
            ...
            if valid_ask and valid_buy:
                ...
                if traderObject.get('b1_last_price', None) == None:
                    mmmid_price = (best_ask + best_bid) / 2
                else:
                    mmmid_price = traderObject['b1_last_price']

            if traderObject.get('b1_last_price', None) is None:
                fair = mmmid_price
            else:
                ### Alpha-ish - LR forecast
                last_price = traderObject["b1_last_price"]
                last_returns = (mmmid_price - last_price) / last_price
                pred_returns = (last_returns * self.params[Product.PICNIC_BASKET1]["reversion_beta"])
                fair = mmmid_price + (mmmid_price * pred_returns)

            if traderObject.get("b2_last_price", None) is not None:
                ### Alpha - Neg Corr Ink
                old_b2_price = traderObject["b2_last_price"]
                valid_ask_b2 = [price for price in b2_order_depth.sell_orders.keys()
                                if abs(b2_order_depth.sell_orders[price]) >= self.params[Product.PICNIC_BASKET2][
                                    "adverse_volume"]]
                valid_buy_b2 = [price for price in b2_order_depth.buy_orders.keys()
                                if abs(b2_order_depth.buy_orders[price]) >= self.params[Product.PICNIC_BASKET2][
                                    "adverse_volume"]]
                if valid_ask_b2 and valid_buy_b2:
                    new_b2_mid = (min(valid_ask_b2) + max(valid_buy_b1)) / 2
                else:
                    new_b2_mid = (min(b2_order_depth.sell_orders.keys()) +
                                  max(b2_order_depth.buy_orders.keys())) / 2

                ink_return = (new_b2_mid - old_b2_price) / old_b2_price
                fair = fair - (self.params[Product.PICNIC_BASKET1].get("b2_adjustment_factor",
                                                                       0.5) * ink_return * mmmid_price)
                #Use LR forcasts for basket 2 model
            ...
            # traderObject["prev_ink_price"] = traderObject.get("ink_last_price", mmmid_price)
            traderObject["b1_last_price"] = mmmid_price
            return fair
        return None
